...
export const FinancesTab: React.FC = () => {
  ...
  const getWavePaymentLink = (amount?: number) => {
    ...
    const customMerchant = (
      ...
      metaEnv.VITE_WAVE_PAYMENT_URL ||
      ''
    ).trim();

    if (customMerchant && customMerchant.startsWith('http')) {
  ...
  const trancheCards = [
    ...
    },
    {
      ...
      subtitle: activeSoiree ? 'Retrouvailles & Dîner de Gala (Événement Fixe)' : 'Aucun événement fixe publié pour le moment',
      eventDate: activeSoiree?.eventDate || '',
      paymentDeadline: activeSoiree?.eventDate || '',
      isActive: !!activeSoiree,
      emptyNotice: 'Aucun événement fixe publié pour le moment',
      progress: soireeProgress,
      accentColor: 'from-emerald-500/10 to-emerald-600/5',
      borderColor: activeSoiree ? 'border-emerald-400/40' : 'border-slate-300',
